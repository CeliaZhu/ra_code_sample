...
summary_table <- analysis_data %>% 
  ...
  rename(Characteristic = skim_variable, Mean = numeric.mean, sd = numeric.sd, Min = numeric.p0, Max = numeric.p100) %>% 
  mutate(Characteristic = str_to_title(Characteristic)) %>% 
  mutate(Characteristic = if_else(Characteristic == "Prior_arrests", "Number of prior arrests", Characteristic)) 
...
# Label variables and values for a cleaner exhibition
...
tbl_summary(
  analysis_data_labeled %>%
    select(prior_arrests, age, treat, black, asian, white, male, female) ,
  by = treat,
  statistic = list(all_continuous() ~ "{mean}({sd})"), 
  digits = list(c(prior_arrests, age, black, asian, white, male, female) ~2)
) %>%
  add_p() %>%
  as_gt() 
```
...
# Specification 1: OLS
ols <- lm_robust(re_arrest ~ treat + prior_arrests + male + black + white + age, data = analysis_data_labeled)
tb1 <- tbl_regression(ols)

# Specification 2: logit
logit <- glm(re_arrest ~ treat + prior_arrests + male + black + white + age, data = analysis_data_labeled, family = binomial)
tb2 <- tbl_regression(logit)

# Specification 3: Propensity score matching (weighting)
pscore_estimate <- glm(treat ~ prior_arrests + male + black + white + age, data = analysis_data_labeled, family = binomial)
pscore <- predict(pscore_estimate, type = "response")

analysis_data_labeled$pscore <- pscore

analysis_data_labeled$ate_weight <- if_else(analysis_data_labeled$treat == 1, (1 / analysis_data_labeled$pscore), (1 / (1 - analysis_data_labeled$pscore)))
analysis_data_labeled$atet_weight <- if_else(analysis_data_labeled$treat == 1, 1, (analysis_data_labeled$pscore / (1 - analysis_data_labeled$pscore)))

psm_ate <- glm(re_arrest ~ treat, data = analysis_data_labeled, weights = ate_weight, family = binomial)
psm_atet <- glm(re_arrest ~ treat, data = analysis_data_labeled, weights = atet_weight, family = binomial)
tb3 <- tbl_regression(psm_ate)
tb4 <- tbl_regression(psm_atet)

# Merge regression table for HTML exhibition 
